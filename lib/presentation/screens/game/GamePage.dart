...
import 'package:blindsplay/presentation/ui/widgets/common.dart';
...
import '../../ui/widgets/player_waiting_ui.dart';
        ...
        builder: (context, state) {
          if (state is GameInitial) {
            ...
            return const FadeInWidget(
                key: Key("GameWaiting"),
                child: PlayerWaitingUi(messages: [
                  'Searching...',
                  'Taking longer than usual...',
                  'Almost there...'
                ], you: null, opponent: null));
          } else if (state is GameWaiting) {
            amplitude.logEvent("Game State GameWaiting");

            return FadeInWidget(
                key: const Key("GameInProgress"),
                child: PlayerWaitingUi(
                    messages: ['Game starts in : ${state.countdown}s'],
                    you: state.you,
      ...
      return BlocBuilder<GameBloc, GameState>(
        builder: (context, state) {
          if (state is GameInitial) {
            ...
            return FadeInWidget(
                key: const Key("GameWaiting"),
                child: MessageUi('Getting ready...'));
          } else if (state is OfflineGameWaiting) {
            amplitude.logEvent("Game State GameWaiting");

            return FadeInWidget(
                key: const Key("GameInProgress"),
                child: MessageUi('Game starts in : ${state.countdown}s'));
          } else if (state is GameInProgress) {
            amplitude.logEvent("Game State GameInProgress");
